...
describe('isAllowedCorsOrigin', () => {
  ...
  it('permite requisicoes sem Origin e dominios configurados', () => {
    ...
    expect(
      isAllowedCorsOrigin(
        'https://admin-test-setta.varten.com.br',
        configured,
      ),
    ).toBe(true);
  });
  ...
  it('permite loopback em qualquer porta para o Flutter local', () => {
    expect(isAllowedCorsOrigin('http://localhost:55165', configured)).toBe(true);
  ...
  it('rejeita origens externas nao configuradas', () => {
    ...
    expect(isAllowedCorsOrigin('https://localhost.example.com', configured)).toBe(
      false,
    );
  });
});
